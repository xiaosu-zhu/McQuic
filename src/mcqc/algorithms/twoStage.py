...
class TwoStage(Algorithm):
    def __init__(self, rank: int, worldSize: int, config: Config, model: Whole, device: str, optimizer: Callable[[Iterator[nn.Parameter]], torch.optim.Optimizer], scheduler: Callable[[torch.optim.Optimizer], torch.optim.lr_scheduler._LRScheduler], saver: Saver, continueTrain: bool, logger: Logger):
        super().__init__()
        self._model = DistributedDataParallel(model.to(rank))
        self._device = device
            ...
            for images in trainLoader:
                ...
                with torch.no_grad():
                    ...
                    if self._saver is not None:
                        # group = dist.new_group(list(range(self._worldSize)))
                        # dist.gather(ssimLoss,)
