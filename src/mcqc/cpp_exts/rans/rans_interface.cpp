...
/********************* NOTE: 16 CHANGED TO 32 *********************/
constexpr int precision = 32;
/********************* NOTE: 16 CHANGED TO 32 *********************/

/********************* NOTE: 4 CHANGED TO 8 *********************/
constexpr uint16_t bypass_precision = 8; /* number of bits in bypass mode */
...
void BufferedRansEncoder::encode_with_indexes(
  ...
  for (size_t i = 0; i < symbols.size(); ++i) {
    ...
    _syms.push_back({static_cast<uint32_t>(cdf[value]),
    ...
    if (value == max_value) {
      ...
      _syms.push_back(
          {static_cast<uint32_t>(val), static_cast<uint32_t>(val + 1), true});
...
/*
PYBIND11_MODULE(ans32, m) {
  m.attr("__name__") = "mcqc.ans32";
