...
class FullDiscriminator(nn.Module):
    def __init__(self, channel):
        super().__init__()
        start = 64
        # sequence = conv(3, 64, 4, 2, 1) + convWithNorm(64, 128, 4, 2, 1) + convWithNorm(128, 256, 4, 2, 1) + convWithNorm(256, 512, 4, 2, 1) + convWithNorm(512, 4, 1, 1) + conv()
        self._net = nn.Sequential(
            ResidualBNBlockWithStride(3, start, stride=2),
            ResidualBNBlock(start, start * 2),
            # AttentionBlock(channel),
            ResidualBNBlockWithStride(start * 2, start * 2, stride=2),
            ResidualBNBlock(start * 2, start * 4),
            # AttentionBlock(channel),
