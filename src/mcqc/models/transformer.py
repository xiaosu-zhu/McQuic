...
class MHAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        v = self.wv(v)
        # [n, nHead, seq, depth]
        q = self._splitHeads(q, n)
        k = self._splitHeads(k, n)
        v = self._splitHeads(v, n)

        attention = self._scaledDotProductAttention(q, k, v, self._scale, mask)
        # [n, seq, dIn]
        attention = attention.permute(0, 2, 1, 3).reshape(n, -1, self._dIn)
    ...
    def _scaledDotProductAttention(q, k, v, scale, mask):
        ...
        # 相加等于1。
        weights = torch.softmax(qkLogits, -1)    # (..., seq_len_q, seq_len_k)
...
class EncoderLayer(nn.Module):
    def __init__(self, dIn, numHeads, dHidden, rate=0.1):
        ...
        self._dropout1 = nn.Dropout(rate, True)
        self._dropout2 = nn.Dropout(rate, True)
...
class DecoderLayer(nn.Module):
    def __init__(self, dIn, numHeads, dHidden, rate=0.1):
        ...
        self._dropout1 = nn.Dropout(rate, True)
        self._dropout2 = nn.Dropout(rate, True)
        self._dropout3 = nn.Dropout(rate, True)
