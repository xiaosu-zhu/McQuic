    ...
    def train(self, trainLoaderFn: Callable[[], DataLoader], *_, beforeRunHook: Optional[Callable] = None, afterRunHook: Optional[Callable] = None, stepStartHook: Optional[Callable] = None, stepFinishHook: Optional[Callable] = None, **__):
        ...
        while True:
            ...
            for images in trainLoader:
                ...
                self._stepFinish(stepFinishHook, reconLoss=reconLoss, mseLoss=mseLoss, lpipsLoss=lpipsLoss, f1_featmap_mse=f1_featmap_mse, q_disLoss=q_disLoss, codes=codes, images=images, restored=xHat, logits=logits, norm=norm, **trainingArgs)
                del images
...
class MainTrainer(_baseTrainer):
    ...
    def _stepFinishHook(self, *_, reconLoss, lpipsLoss, mseLoss, f1_featmap_mse, q_disLoss, norm, **__):
        ...
        if self._step % (self.config.Train.ValFreq // 1000) != 0:
            return
        if self.rank == 0:
            wandb.log({f"Stat/Loss_{self.config.Train.Target}": distortionDB, "Stat/Loss_lpips": lpipsLoss, "Stat/Loss_mse": mseLoss, "Stat/Loss_f1_mse": f1_featmap_mse, "Stat/Loss_q_dis": q_disLoss, "Stat/Lr": self._scheduler.get_last_lr()[0], "Stat/Norm": norm}, step=self._step)
