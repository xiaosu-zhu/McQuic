...
def getTrainLoader(gen: bool, datasetPath: StrPath, batchSize: int, logger: Union[logging.Logger, LoggerBase] = logging.root):
    ...
    trainLoader = DataLoader(trainDataset, batch_size=None, num_workers=max(batchSize // 2, 8) if gen else max(batchSize + 4, 4), pin_memory=True, persistent_workers=False)
    return trainLoader
